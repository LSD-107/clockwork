...
function updateButtonStates() {
    const buttons = document.querySelectorAll('.shop .building button'); // Sélectionner tous les boutons des bâtiments
    buttons.forEach(button => {
        const buildingId = button.id.replace('b', ''); // Récupérer l'ID du bâtiment (b1, b2, etc.)
        const cost = buildingPrices[Object.keys(buildingPrices)[buildingId - 1]]; // Récupérer le coût correspondant au bâtiment

        // Comparer les points disponibles avec le coût
        if (counter >= cost) {
            button.disabled = false; // Activer le bouton
            button.style.backgroundColor = "white"; // Couleur blanche pour les bâtiments achetables
            button.style.color = "black"; // Texte noir pour plus de contraste
            button.style.cursor = "pointer"; // Curseur par défaut
        } else {
            button.disabled = true; // Désactiver le bouton
            button.style.backgroundColor = "lightgray"; // Couleur gris clair
            button.style.color = "gray"; // Texte grisé
            button.style.cursor = "not-allowed"; // Curseur désactivé
        }
    });
}
    ...
    buyBuilding('montre-de-poche', buildingPrices['montre-de-poche'], 47);
		...
		function createRandomButton() {
			...
			const x = Math.random() * window.innerWidth - 100; // x aléatoire
			const y = Math.random() * window.innerHeight - 100; // y aléatoire
		...
		function randomButtonGenerator(){ 
			const randomTime = Math.random() *  600000; //minutes
			setTimeout(() => {
				createRandomButton();  
